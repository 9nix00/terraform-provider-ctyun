...
resource "ctyun_ecs" "ecs_test" {
  ...
  security_group_ids = [ctyun_security_group.sg_test.id]
  is_destroy_instance = true
}

resource "ctyun_scaling_ecs_protection" "scaling_ecs_protect_example" {
  group_id         = ctyun_scaling_group.group_test.id
  instance_id_list = [ctyun_ecs.ecs_test.id]
  protect_status   = true
}