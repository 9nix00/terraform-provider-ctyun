...
func NewCcseUpdateSecretApi(client *core.CtyunClient) *CcseUpdateSecretApi {
	return &CcseUpdateSecretApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPut,
			UrlPath:      "/v2/cce/clusters/{clusterId}/api/v1/namespaces/{namespaceName}/secrets/{secretName}",
...
func (a *CcseUpdateSecretApi) Do(ctx context.Context, credential core.Credential, req *CcseUpdateSecretRequest) (*CcseUpdateSecretResponse, error) {
	...
	builder = builder.ReplaceUrl("secretName", req.SecretName)
	...
	var resp CcseUpdateSecretResponse
