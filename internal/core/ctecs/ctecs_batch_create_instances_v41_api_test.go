...
func TestCtecsBatchCreateInstancesV41Api_Do(t *testing.T) {
	// 初始化
	client := core.DefaultClient()
	credential := core.NewCredential("<YOUR_AK>", "<YOUR_SK>")
	// credential := core.CredentialFromEnv()
	apis := NewApis("<YOUR_ENDPOINT>", client)
	api := apis.CtecsBatchCreateInstancesV41Api

	// 构造请求
	var bootDiskIsEncrypt bool = true
	var monitorService bool = true
	var isEncrypt bool = true
	request := &CtecsBatchCreateInstancesV41Request{
		...
		OnDemand:          false,
		NetworkCardList: []*CtecsBatchCreateInstancesV41NetworkCardListRequest{
			{
		...
		SecGroupList: []string{},
		DataDiskList: []*CtecsBatchCreateInstancesV41DataDiskListRequest{
			{
				DiskMode:  "VBD",
				DiskType:  "SATA",
				DiskSize:  20,
				CmkID:     "9dcd1cf7-7969-4b3b-86a1-31f79682e219",
				IsEncrypt: &isEncrypt,
				DiskName:  "ebs.name",
			},
		},
		...
		LabelList: []*CtecsBatchCreateInstancesV41LabelListRequest{
			{
				LabelKey:   "test-key",
