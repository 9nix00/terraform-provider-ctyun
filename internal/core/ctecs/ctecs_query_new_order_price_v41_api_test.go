...
func TestCtecsQueryNewOrderPriceV41Api_Do(t *testing.T) {
	// 初始化
	client := core.DefaultClient()
	credential := core.NewCredential("<YOUR_AK>", "<YOUR_SK>")
	// credential := core.CredentialFromEnv()
	apis := NewApis("<YOUR_ENDPOINT>", client)
	api := apis.CtecsQueryNewOrderPriceV41Api
	...
	request := &CtecsQueryNewOrderPriceV41Request{
		...
		SysDiskType:  "SATA",
		SysDiskSize:  50,
		Disks: []*CtecsQueryNewOrderPriceV41DisksRequest{
			{
				DiskType: "SATA",
				DiskSize: 10,
			},
		...
		AzName:          "az1",
		OrderDisks: []*CtecsQueryNewOrderPriceV41OrderDisksRequest{
			{
				DiskType: "SATA",
				DiskSize: 10,
			},
