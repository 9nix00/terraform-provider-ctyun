package sdwan

import (
	"context"
	"github.com/ctyun-it/terraform-provider-ctyun/internal/core/core"
	"net/http"
)

// SdwanCreateSdwanAclApi
/* 创建访问控制实例 */
type SdwanCreateSdwanAclApi struct {
	template core.CtyunRequestTemplate
	client   *core.CtyunClient
}

func NewSdwanCreateSdwanAclApi(client *core.CtyunClient) *SdwanCreateSdwanAclApi {
	return &SdwanCreateSdwanAclApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodPost,
			UrlPath:      "/v4/sdwan/acl/create",
			ContentType:  "application/json",
		},
	}
}

func (a *SdwanCreateSdwanAclApi) Do(ctx context.Context, credential core.Credential, req *SdwanCreateSdwanAclRequest) (*SdwanCreateSdwanAclResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	_, err := ctReq.WriteJson(struct {
		*SdwanCreateSdwanAclRequest
	}{
		req,
	}, a.template.ContentType)
	if err != nil {
		return nil, err
	}
	response, err := a.client.RequestToEndpoint(ctx, ctReq)
	if err != nil {
		return nil, err
	}
	var resp SdwanCreateSdwanAclResponse
	err = response.Parse(&resp)
	if err != nil {
		return &resp, err
	}
	return &resp, nil
}

type SdwanCreateSdwanAclRequest struct {
	AclName   string                             `json:"aclName"`         /*  名称  */
	ProjectID string                             `json:"projectID"`       /*  企业项目  */
	Rules     []*SdwanCreateSdwanAclRulesRequest `json:"rules,omitempty"` /*  rules  */
}

type SdwanCreateSdwanAclRulesRequest struct {
	Direction    string `json:"direction"`    /*  本参数表示控制方向<br/><br/>取值范围:<br/>in:入方向<br/>out:出方向  */
	Protocol     string `json:"protocol"`     /*  本参数表示协议类型<br/><br/>取值范围:<br/>udp:UDP<br/>icmp:ICMP</br>all:ALL</br>tcp:TCP  */
	IpVersion    string `json:"ipVersion"`    /*  本参数表示IP协议版本<br/><br/>取值范围:<br/>IPv4:IPv4<br/>IPv6:IPv6  */
	DstCidr      string `json:"dstCidr"`      /*  目的网段  */
	DstPortRange string `json:"dstPortRange"` /*  目的端口范围（例如1-200， -1/-1为默认值，表示1-65535）  */
	Priority     string `json:"priority"`     /*  priority  */
	Action       string `json:"action"`       /*  本参数表示策略类型<br/><br/>取值范围:<br/>allow:允许<br/>deny:拒绝  */
	SrcCidr      string `json:"srcCidr"`      /*  源网段  */
	SrcPortRange string `json:"srcPortRange"` /*  源端口范围（例如1-200， -1/-1为默认值，表示1-65535）  */
}

type SdwanCreateSdwanAclResponse struct {
	StatusCode  int32   `json:"statusCode"`  /*  返回状态码('800为成功，900为失败)  ，默认值:800  */
	ErrorCode   *string `json:"errorCode"`   /*  业务细分码，为product.module.code三段式码  */
	Message     *string `json:"message"`     /*  失败时的错误描述，一般为英文描述  */
	Description *string `json:"description"` /*  失败时的错误描述，一般为中文描述  */
	OperationID *string `json:"operationID"` /*  操作id  */
	Error       *string `json:"error"`       /*  业务细分码，为product.module.code三段式码  */
}
