...
type EbsCreateEbsSnapRequest struct {
	...
	●custom：自定义保留天数。
	●forever：永久保留。  */
	RetentionTime int32 `json:"retentionTime"` /*  自定义快照保留天数。取值范围：1-65535。当快照保留策略为custom时该参数为必填，当快照保留策略为forever时，自动设置为65535。  */
}
...
type EbsCreateEbsSnapReturnObjResponse struct {
	OrderID      *string `json:"orderID,omitempty"`      /*  是否为主订单。  */
	ResourceType *string `json:"resourceType,omitempty"` /*  云硬盘快照资源类型EBS（只有一种）。  */
