package nat

import (
	"context"
	"terraform-provider-ctyun/internal/core/core"
	"testing"
)

func TestCtvpcCreateEipWithIpAddressApi_Do(t *testing.T) {
	// 初始化
	client := core.DefaultClient()
	credential := core.NewCredential("<YOUR_AK>", "<YOUR_SK>")
	// credential := core.CredentialFromEnv()
	apis := NewApis("<YOUR_ENDPOINT>", client)
	api := apis.CtvpcCreateEipWithIpAddressApi

	// 构造请求
	var projectID string = ""
	var bandwidthID string = ""
	var demandBillingType string = "bandwidth"
	var lineType string = "bgp"
	var payVoucherPrice string = "1"
	var segmentID string = ""
	var exclusiveName string = ""
	request := &CtvpcCreateEipWithIpAddressRequest{
		ClientToken:       "79fa97e3-c48b-xxxx-9f46-6a13d8163678",
		RegionID:          "",
		ProjectID:         &projectID,
		CycleType:         "on_demand",
		CycleCount:        1,
		Name:              "",
		Bandwidth:         1,
		IpAddress:         "",
		BandwidthID:       &bandwidthID,
		DemandBillingType: &demandBillingType,
		LineType:          &lineType,
		PayVoucherPrice:   &payVoucherPrice,
		SegmentID:         &segmentID,
		ExclusiveName:     &exclusiveName,
	}

	// 发起调用
	response, err := api.Do(context.Background(), *credential, request)
	if err != nil {
		t.Log("request error:", err)
		t.Fail()
		return
	}
	t.Logf("%+v\n", *response)
}
