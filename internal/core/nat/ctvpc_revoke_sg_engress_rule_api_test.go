...
func TestCtvpcRevokeSgEngressRuleApi_Do(t *testing.T) {
	// 初始化
	client := core.DefaultClient()
	credential := core.NewCredential("<YOUR_AK>", "<YOUR_SK>")
	// credential := core.CredentialFromEnv()
	apis := NewApis("<YOUR_ENDPOINT>", client)
	api := apis.CtvpcRevokeSgEngressRuleApi

	// 构造请求
	request := &CtvpcRevokeSgEngressRuleRequest{
		RegionID:            "79fa97e3-c48b-xxxx-9f46-6a13d8163678",
		SecurityGroupID:     "sg-bp67axxxxzb4p",
		SecurityGroupRuleID: "sg-bp67axxxxzb4p",
