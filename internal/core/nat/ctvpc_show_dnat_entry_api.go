...
func NewCtvpcShowDnatEntryApi(client *core.CtyunClient) *CtvpcShowDnatEntryApi {
	return &CtvpcShowDnatEntryApi{
		client: client,
		template: core.CtyunRequestTemplate{
			EndpointName: EndpointName,
			Method:       http.MethodGet,
			UrlPath:      "/v4/vpc/detail-dnat-entries",
			ContentType:  "application/json",
		},
	}
}

func (a *CtvpcShowDnatEntryApi) Do(ctx context.Context, credential core.Credential, req *CtvpcShowDnatEntryRequest) (*CtvpcShowDnatEntryResponse, error) {
	builder := core.NewCtyunRequestBuilder(a.template)
	builder.WithCredential(credential)
	ctReq := builder.Build()
	ctReq.AddParam("regionID", req.RegionID)
	ctReq.AddParam("natGatewayID", req.NatGatewayID)
	ctReq.AddParam("dNatID", req.DNatID)
	response, err := a.client.RequestToEndpoint(ctx, ctReq)
	if err != nil {
		return nil, err
	}
	var resp CtvpcShowDnatEntryResponse
