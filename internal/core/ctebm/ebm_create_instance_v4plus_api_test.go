...
func TestEbmCreateInstanceV4plusApi_Do(t *testing.T) {
	// 初始化
	client := core.DefaultClient()
	credential := core.NewCredential("<YOUR_AK>", "<YOUR_SK>")
	// credential := core.CredentialFromEnv()
	apis := NewApis("<YOUR_ENDPOINT>", client)
	api := apis.EbmCreateInstanceV4plusApi
	...
	request := &EbmCreateInstanceV4plusRequest{
		...
		DiskList: []*EbmCreateInstanceV4plusDiskListRequest{
			{
				DiskType: "data",
				Title:    &title,
				RawType:  "SSD",
		...
		NetworkCardList: []*EbmCreateInstanceV4plusNetworkCardListRequest{
			{
				Title:    &title1,
				FixedIP:  &fixedIP,
				Master:   true,
