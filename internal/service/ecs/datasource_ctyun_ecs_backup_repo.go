...
var (
	_ datasource.DataSource              = &ctyunEcsBackupRepos{}
	_ datasource.DataSourceWithConfigure = &ctyunEcsBackupRepos{}
)

type ctyunEcsBackupRepos struct {
	meta *common.CtyunMetadata
}

func NewCtyunEcsBackupRepos() datasource.DataSource {
	return &ctyunEcsBackupRepos{}
}

func (c *ctyunEcsBackupRepos) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_ecs_backup_repos"
}
...
type ctyunEcsBackupReposConfig struct {
	...
	BackupRepos    []ctyunEcsBackupReposModel `tfsdk:"backup_repos"`
}
...
func (c *ctyunEcsBackupRepos) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	var err error
	defer func() {
		if err != nil {
	...
	var config ctyunEcsBackupReposConfig
	...
	config.BackupRepos = []ctyunEcsBackupReposModel{}
	// 组装请求体
	params := &ctecs2.CtecsListInstanceBackupRepoRequest{
		RegionID:       config.RegionID.ValueString(),
		ProjectID:      config.ProjectID.ValueString(),
		RepositoryName: config.RepositoryName.ValueString(),
	...
	// 解析返回值
	for _, repo := range resp.ReturnObj.Results {
		// 处理备份列表
		...
		item := ctyunEcsBackupReposModel{
			...
			RemainingSize:  types.Float64Value(float64(repo.RemainingSize)),
			...
			BackupCount:    types.Int64Value(int64(repo.BackupCount)),
		}
...
func (c *ctyunEcsBackupRepos) Configure(_ context.Context, request datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
