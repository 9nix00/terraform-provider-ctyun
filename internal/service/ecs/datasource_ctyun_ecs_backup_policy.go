...
var (
	_ datasource.DataSource              = &ctyunEcsBackupPolicies{}
	_ datasource.DataSourceWithConfigure = &ctyunEcsBackupPolicies{}
)

type ctyunEcsBackupPolicies struct {
	meta *common.CtyunMetadata
}

func NewCtyunEcsBackupPolicies() datasource.DataSource {
	return &ctyunEcsBackupPolicies{}
}

func (c *ctyunEcsBackupPolicies) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_ecs_backup_policies"
}
...
type ctyunEcsBackupPoliciesConfig struct {
	RegionID       types.String                  `tfsdk:"region_id"`
	ProjectID      types.String                  `tfsdk:"project_id"`
	PolicyName     types.String                  `tfsdk:"name"`
	PolicyID       types.String                  `tfsdk:"policy_id"`
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"cycle_type": schema.StringAttribute{
							Computed:    true,
							Description: "云主机备份周期类型，取值范围：day：按天备份，week：按星期备份",
						},
						...
						"retention_type": schema.StringAttribute{
							Computed:    true,
							Description: "云主机备份保留类型，取值范围：date：按时间保留，num：按数量保留，all：永久保留",
						},
						...
						"adv_retention_status": schema.BoolAttribute{
							Computed:    true,
							Description: "是否开启高级保留策略，false（不启用），true(启用)，默认值为false。需校验云主机备份保留类型（retentionType），若保留类型为按数量保存（num），可开启高级保留策略；若保留类型为date（按时间保存）或all（永久保存），不可开启高级保留策略。",
						},
...
func (c *ctyunEcsBackupPolicies) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	var err error
	defer func() {
		if err != nil {
	...
	var config ctyunEcsBackupPoliciesConfig
	...
	config.BackupPolicies = []ctyunEcsBackupPoliciesModel{}
	// 组装请求体
	...
	for _, policy := range resp.ReturnObj.PolicyList {
		...
		item := ctyunEcsBackupPoliciesModel{
			...
			ResourceCount:      types.Int64Value(int64(policy.ResourceCount)),
			ResourceIDs:        types.StringValue(policy.ResourceIDs),
			ProjectID:          types.StringValue(policy.ProjectID),
			FullBackupInterval: types.Int64Value(int64(policy.FullBackupInterval)),
			AdvRetentionStatus: types.BoolValue(policy.AdvRetentionStatus),
...
func (c *ctyunEcsBackupPolicies) Configure(_ context.Context, request datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
