...
var (
	_ datasource.DataSource              = &CtyunMongodbSpecs{}
	_ datasource.DataSourceWithConfigure = &CtyunMongodbSpecs{}
)
...
func (c *CtyunMongodbSpecs) Metadata(ctx context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_mongodb_specs"
}
func (c *CtyunMongodbSpecs) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	...
	var config CtyunMongodbSpecsConfig
	...
	var specs []CtyunMongodbSpecInfoModel
	for _, specItem := range returnObjData {
		var spec CtyunMongodbSpecInfoModel
...
type CtyunMongodbSpecsConfig struct {
	...
	Specs        []CtyunMongodbSpecInfoModel `tfsdk:"specs"`
}
