...
func TestAccCtyunBackupRepo(t *testing.T) {
	...
	resourceName := "ctyun_ebs_backup_repo." + rnd
	datasourceName := "data.ctyun_ebs_backup_repos." + dnd
	resourceFile := "resource_ctyun_ebs_backup_repo.tf"
	datasourceFile := "datasource_ctyun_ebs_backup_repos.tf"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// 更新
			{
				Config: utils.LoadTestCase(resourceFile, rnd, updatedName, initSize),
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			// 扩容
			{
				Config: utils.LoadTestCase(resourceFile, rnd, updatedName, updatedSize),
				Check: resource.ComposeAggregateTestCheckFunc(
