resource "ctyun_vpc" "vpc_test" {
  name        = "tf-vpc-for-unified-gateway"
  cidr        = "192.168.0.0/16"
...
resource "ctyun_subnet" "subnet_test" {
  vpc_id = ctyun_vpc.vpc_test.id
  name        = "tf-subnet-for-unified-gateway"
  cidr        = "192.168.1.0/24"
  description = "terraform测试使用"
  dns         = [
    "114.114.114.114",
...
resource "ctyun_ecs" "ecs_test" {
  instance_name       = "tf-ecs-for-cc-1"
  display_name        = "tf-ecs-for-cc-1"
...
resource "ctyun_ecs" "ecs_test2" {
  instance_name       = "tf-ecs-for-www-2"
  display_name        = "tf-ecs-for-www-2"
  flavor_id           = data.ctyun_ecs_flavors.ecs_flavor_test.flavors[0].id
  image_id            = data.ctyun_images.image_test.images[0].id
  system_disk_type    = "sata"
  system_disk_size    = 40
